...
namespace destroyer {
    ...
    Real VDBSampler::Sample(Real x, Real y, Real z) const {
        ...
        UT_Vector3 p(x,y,z);
        Real sample_val = vdb_->getValueF(p);

        if (apply_fit_) {
            sample_val = (sample_val - fit_min_)  / (fit_max_ / fit_min_);
            sample_val = (sample_val>1.0) ? 1.0 : sample_val;
            sample_val = (sample_val<0.0) ? 0.0 : sample_val;
        }
